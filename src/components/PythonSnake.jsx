  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      for (let i = segs.length - 1; i >= 0; i--) {
        ...
        const r = 11 * (1 - t * 0.55)
        // Neon hue cycling cyan → magenta along body
        const hue = 180 + (i / segs.length) * 140 + Math.sin(tRef.current * 0.04) * 8
        const stripe = i % 3 === 0
        ctx.beginPath()
        ctx.arc(s.x, s.y, r, 0, Math.PI * 2)
        const grad = ctx.createRadialGradient(s.x - r * 0.4, s.y - r * 0.4, 0, s.x, s.y, r)
        grad.addColorStop(0, `hsla(${hue}, 95%, ${stripe ? 70 : 55}%, 0.98)`)
        grad.addColorStop(1, `hsla(${hue}, 90%, 18%, 0.85)`)
        ctx.shadowColor = `hsla(${hue}, 95%, 60%, 0.9)`
        ctx.shadowBlur = stripe ? 14 : 6
        ctx.fillStyle = grad
        ctx.fill()
        // soft outline
